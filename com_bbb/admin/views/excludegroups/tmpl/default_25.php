...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JFactory::getApplication()->input->get('function', null)) {
 	...
 	$link = JRoute::_( 'index.php?option=com_bbb&view=meeting&task=meeting.edit&id='. $row->id );
 	$row->id = $row->id;
  ...
  ?>
	...
	<td style="text-align: center;">
			...
			<td style="text-align: center;"><?php if ($bbb->isMeetingRunning($row->id) == 'true') {
								echo "Yes (<a href='".JURI::base()."index.php?option=com_bbb&task=endMeeting&meetingId=".$row->id."'>End Now</a>)";
							}
							else {
								echo "No";
							} ?></td>		
