...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_bbb&task=categories.saveOrderAjax&tmpl=component';
...
?>
<form action="index.php?option=com_bbb&view=category" method="post" name="adminForm" id="adminForm">
  ...
  if (count($this->items)) : 
  		foreach ($this->items as $i => $item) :
    			...
    			if (JFactory::getApplication()->input->get('function', null)) {
    				$onclick= "onclick=\"window.parent.jSelectCategory_id('".$item->id."', '".$this->escape($item->title)."', '','id')\" ";
    			}  	
    
 				$link = JRoute::_( 'index.php?option=com_bbb&view=category&task=category.edit&id='. $item->id );
