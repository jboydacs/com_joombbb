...
class BBBModelType  extends JModelAdmin { 
...
/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_bbb.type', 'type', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_bbb.edit.type.data', array());
		...
		if(!version_compare(JVERSION,'3','<')){
			$this->preprocessData('com_bbb.type', $data);
		}
	...
	public function save(array $data)
	{
		...
		if($id){
			...
			$updateBbbCats = $db->updateObject('#__bigbluebutton_type', $bbbCats, 'id');
		} else {
			$bbbCats = new stdClass();
			$bbbCats->title = $title;
			
			// Insert the objects into the database.
			$result = $db->insertObject('#__bigbluebutton_type', $bbbCats);
			$thisId = $db->insertid();
		}
		...
		if($task == 'apply'){
			$url = JRoute::_('/administrator/index.php?option=com_bbb&view=type&layout=edit&id=' . (int)$thisId);
		} else {
			$url = JRoute::_('/administrator/index.php?option=com_bbb&view=types');
		}
