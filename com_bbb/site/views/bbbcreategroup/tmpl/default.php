...
<style>
	...
	.grouptitle_desc_cont input[type=text], .grouptitle_desc_cont textarea {
		border: 1px solid #A0C843;
		width: 75%;
		vertical-align: middle;
		display: inline-block;
		margin: 0;
	}
	.grouptitle_desc_cont {
		display: inline-block;
	...
	#selected_user_id, #selected_user_id_chzn{
		width: 230px;
		text-align: left;
	}
	.add_usertogroup_btn{
...
<script>
	jQuery(document).ready(function(){
		...
		jQuery('.add_usertogroup_btn').click(function(){
				
				/* jQuery('#selectedusers_cont'). */
				
				var inputidentity = jQuery('#selected_user_id').val()+''+jQuery('#selected_user_id option:selected').text().replace(/[.# ]/g,"_");
				
				if(jQuery('#selectedusers_cont input').hasClass(inputidentity) || jQuery('#selected_user_id').val() == 0){
					return false;
				}
				
				jQuery('<div/>', {
					id: "selectedusermaincont"+inputidentity,
					class: "selectedusermaincont",
					html: "&nbsp;"
				}).appendTo(jQuery('#selectedusers_cont'));
				
				jQuery('<div/>', {
					id: 'div'+inputidentity,
					class: inputidentity+' selectedusercont',
					html: jQuery('#selected_user_id option:selected').text()
				}).appendTo(jQuery("#selectedusermaincont"+inputidentity));
				
				jQuery('<i/>', {
					class: "fa fa-times removebtn",
					html: "&nbsp;"
				}).appendTo(jQuery("#selectedusermaincont"+inputidentity));
				
				jQuery('<input>', {
					id: 'input'+inputidentity,
					type: 'hidden',
					name: 'selected_usersid[]',
					class: inputidentity,
					value: jQuery('#selected_user_id').val()
				}).appendTo(jQuery("#selectedusermaincont"+inputidentity));
				
				jQuery('.selectedusers_cont .removebtn').each(function(){
					jQuery(this).click(function(){
						jQuery(this).parent('.selectedusermaincont').remove();
					});
				});
		});
		...
		jQuery('.selectedusers_cont .removebtn').each(function(){
			jQuery(this).click(function(){
				alert(1);jQuery(this).parent('.selectedusermaincont').remove();
			});
		});
...
<?php } ?>
	...
	<div class="bbb-container" id="bbb-container"> 	
		<h2 class="icomponent_title">Groups</h2>
	
		<div class="createlesson_btn greenbutton mainbuttons">Create Lessons</div>
		<div class="creategroups_btn greenbutton mainbuttons">Create Groups</div>
		<div class="privatelessons_btn greenbutton mainbuttons">Private lessons</div>
	
		<div class="newlesson_btn">New Group</div>

		<form action="<?php echo $pageURL; ?>" method="post" name="saveform" id="saveform" class="lessonform">
			
			<div class="grouptitle_desc_cont">
				<input aria-invalid="false" required="" aria-required="true" type="text" name="lesson_title" id="lesson_title" placeholder="Group Name" />
				<input type="submit" value="save" id="submitbutton" />
			</div>
			<div class="addusers_cont">
				<?php echo $this->selectedUserDropdownList; ?>
				<div class="add_usertogroup_btn">Add</div>
			</div>
			
			<div class="selectedusers_cont" id="selectedusers_cont">
